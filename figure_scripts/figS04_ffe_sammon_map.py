"""
Supplementary Figure S4 — Sammon / MDS Map of Flash-Flood-Episode SOM

Single-panel figure showing the Sammon projection of the 2×2 FFE SOM.
Nodes are colored by the U-matrix (mean distance to neighbors) and sized by
hit frequency (number of FFEs assigned). Topology-preserving neighbor edges
are drawn as faint gray lines.

Usage:
    python -m figure_scripts.figS04_ffe_sammon_map
"""
...
def main():
    ...
    coords   = cached["coords"]     # (n_nodes, 2)
    ...
    for idx, (x, y) in enumerate(coords):
        ...
        ax.text(
            ...
            fontsize=5.5, ha="center", va="center", zorder=5,
            path_effects=[
    ...
    for hv in hit_vals:
        ...
        h = ax.scatter([], [], s=s, c="0.55", edgecolors="k",
                       linewidths=0.4, label=f"{hv:d} FFEs")
    ...
    # ── Save ─────────────────────────────────────────────────────────────────
    base = os.path.join(OUT_DIR, "figS04_ffe_sammon_map")
