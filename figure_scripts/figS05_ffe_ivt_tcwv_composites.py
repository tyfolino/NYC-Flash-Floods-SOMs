"""
Supplementary Figure S5 — FFE SOM Moisture Composites: IVT (left) and TCWV (right)

4-row × 2-column layout. Each row is one FFE SOM node (A1, A2, B1, B2).
...
Usage:
    python -m figure_scripts.figS05_ffe_ivt_tcwv_composites
"""
...
CACHE_PATH  = "/home/janoski/nyc_flash_flood/figs/Z500-and-thetae-SOM/.cache/som_results.npz"
...
def main():
    ...
    for row, (i, j) in enumerate(NODE_ORDER):
        ...
        ax_ivt.text(0.0, 1.01, lbl, transform=ax_ivt.transAxes,
                    fontsize=5.5, fontweight="bold", ha="left", va="bottom")
        ax_ivt.text(1.0, 1.01, f"$n$={n}", transform=ax_ivt.transAxes,
        ...
        ax_tcwv.text(0.0, 1.01, lbl, transform=ax_tcwv.transAxes,
                     fontsize=5.5, fontweight="bold", ha="left", va="bottom")
        ax_tcwv.text(1.0, 1.01, f"$n$={n}", transform=ax_tcwv.transAxes,
                     fontsize=6.0, ha="right", va="bottom")

    # ── Colorbars ─────────────────────────────────────────────────────────────
    cbar_ivt = fig.colorbar(im_ivt, ax=axes[:, 0].tolist(),
                            orientation="vertical", pad=0.02, shrink=0.7, aspect=25)
    cbar_ivt.set_label(r"$|\mathrm{IVT}|$ (kg m$^{-1}$ s$^{-1}$)", fontsize=6)
    cbar_ivt.ax.tick_params(labelsize=5)

    cbar_tcwv = fig.colorbar(im_tcwv, ax=axes[:, 1].tolist(),
                             orientation="vertical", pad=0.02, shrink=0.7, aspect=25)
    cbar_tcwv.set_label(r"TCWV (kg m$^{-2}$)", fontsize=6)
    cbar_tcwv.ax.tick_params(labelsize=5)

    # ── Save ─────────────────────────────────────────────────────────────────
    base = os.path.join(OUT_DIR, "figS05_ffe_ivt_tcwv_composites")
