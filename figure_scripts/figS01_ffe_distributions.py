...
def load_data():
    ...
    df["BEGIN_DATETIME"] = pd.to_datetime(begin_str, format="%m/%d/%Y %H%M",
                                          errors="coerce")
...
def main():
    ...
    ax1.plot(years, trend_y, color="firebrick", linewidth=0.9, linestyle="--",
    ...
    ax2.bar(df_monthly.index, df_monthly.values, width=0.8, color=BAR_COLOR,
            linewidth=0)
    ...
    # ── (c) Diurnal ───────────────────────────────────────────────────────────
    ax3.bar(df_hourly.index + 0.5, df_hourly.values, width=0.8, color=BAR_COLOR,
            linewidth=0)
    ax3.set_xlim(0, 24)
    ...
    ax3.text(-0.14, 1.02, "(c)", transform=ax3.transAxes,
             fontsize=6.5, fontweight="bold", ha="left", va="bottom")
