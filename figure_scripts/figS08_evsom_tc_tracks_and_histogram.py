"""
Supplementary Figure S8 — evSOM TC Tracks and Node Association Histogram
...
Usage:
    python -m figure_scripts.figS08_evsom_tc_tracks_and_histogram
"""
...
def main():
    ...
    for _, row in tc_events.iterrows():
        ...
        for sid in row["storm_ids"].split(", "):
            ...
            # Full track (faint)
            ax_map.plot(track["LON"], track["LAT"],
            ...
            ax_map.scatter(cp["LON"], cp["LAT"],
                           color=color, s=10, marker="o",
                           edgecolors="k", linewidths=0.3, zorder=5,
                           transform=ccrs.PlateCarree(), **label_kw)
    ...
    ax_map.plot(
        ...
        "k--", lw=0.8, transform=ccrs.PlateCarree(), zorder=4,
    )

    # NYC star
    ax_map.scatter(-74.0, 40.7, color="black", s=30, marker="*",
                   zorder=8, transform=ccrs.PlateCarree())

    # Legend
    legend_handles = [
        plt.Line2D([0], [0], color=NODE_COLORS[(i, j)], lw=1.5,
                   label=node_label(i, j))
        for j in range(YDIM) for i in range(XDIM)
    ] + [
        plt.Line2D([0], [0], color="gray", lw=0.5, alpha=0.4, label="Full track"),
        plt.Line2D([0], [0], color="gray", lw=1.0,             label=r"$\pm$48 h"),
    ]
    ax_map.legend(handles=legend_handles, fontsize=4.5, loc="lower left",
                  framealpha=0.8, ncol=2)

    ax_map.text(-0.02, 1.01, "(a)", transform=ax_map.transAxes,
                fontsize=6.5, fontweight="bold", ha="right", va="bottom")
    ...
    for xi, (pct, tc, tot) in enumerate(zip(tc_pct, tc_counts, totals)):
        ax_bar.text(xi, pct + 0.5, f"{int(tc)}/{int(tot)}",
                    ha="center", va="bottom", fontsize=4.5)

    # Overall TC % dashed line
    ax_bar.axhline(overall_pct, color="k", lw=0.8, ls="--",
    ...
    # ── Save ─────────────────────────────────────────────────────────────────
    base = os.path.join(OUT_DIR, "figS08_evsom_tc_tracks_and_histogram")
